...
server <- function(input, output) {
  ...
  theme_white <- function (base_size = 12, base_family = "Helvetica") {
    ...
    output$PlotCO <- renderPlot({
      ...
      p <- reactive({ggscatter(temp2(), x = "bpm", y = input$z, size = 4, shape = "class", color = "class",
                palette = "dark2", add = "reg.line", conf.int = T) + xlab("bpm") + ylab(input$z) +
          theme_white() +
    ...
    output$plotpsych = renderGauge({
      ...
      dataset_scaled_ps <- reactive({
        tt <- reactive({ names(all23_scaled) %in% names(all23_scaled)[c(20,29,30,31,32,33,34,11,35)] })
        zz <- reactive({ c(input$x, "Sdev", "Mean") })
        ttt1 <- all23_scaled_ms[all23_scaled_ms$ID %in% zz() , ]
        ttt1 <- ttt1[,names(all23_scaled_ms)[tt()]]
      })
    ...
    ttt10 <- reactive({ 
      ...
      names(temp)[1] <- input$x
      temp$vars <- row.names(temp)
      temp %>%
        mutate(condition = ifelse(get(input$PatID) > (Mean + Sdev) | get(input$PatID) < (Mean - Sdev),0,1)) %>%
        select(vars, everything())
    })
    
    output[["Table"]] <- renderDT({
      
      toprint <- reactive({ datatable(ttt10()[,-1], rownames = ttt10()[,1], 
                           options = list(searching = FALSE,
                                          columnDefs = list(list(width = '50px', targets = "_all")))) %>% 
        formatStyle(input$PatID, 'condition', 
    ...
    #   filename = function() {
    #     paste(input$dataset, input$format, sep = "")
    #   },
    #   content = function(file){
